...
void xor_block(hashblock res, const hashblock in1, const hashblock in2)
{
  unsigned int i;
...
void double_block(hashblock res, const hashblock in)
{
  unsigned int i;
  ...
  /* xor P(x) if msb */
  res[OMD_n - 2] = res[OMD_n - 2] ^ (carry & 0x04);
  /* shift and xor P(x) if msb */
  res[OMD_n - 1] = (in[OMD_n - 1] << 1) ^ (carry & 0x25);
}

void triple_block(hashblock res, const hashblock in)
{
  unsigned int i;
  /* create bitmask from msb using signed shift */
  unsigned char carry = (unsigned char)(((char)in[0]) >> 7);
...
void l2b(hashblock bit_string, int value)
{
  int ptr = value * 8;
...
void calc_L_i(hashblock l, const hashblock lzero, unsigned int i)
{
  /* i==0 => l=lzero */
  if (i == 0)
  {
...
void increment_masking_message(hashblock delta_n,
                               const hashblock delta_o,
                               const hashblock lzero,
                               int i)
{
  hashblock l;
...
void final_masking_message(hashblock delta_n,
                           const hashblock delta_o,
...
void increment_masking_associated_data(hashblock delta_n,
                                       const hashblock delta_o,
                                       const hashblock lzero,
                                       int i)
{
...
void final_masking_associated_data(hashblock delta_n,
                                   const hashblock delta_o,
                                   const hashblock lstar)
{
  /* \delta_{i, 0} = \delta_{i-1, 0} ^ L[ntz(i)]  */
          ...
          unsigned long long int adlen)
{
  ...
  else
  {
    ...
    memcpy(
        right, &ad[(l - 1) * b + OMD_n], (modlen < OMD_n ? 0 : modlen - OMD_n));
...
int omdsha256_process(unsigned char* data,
                      ...
                      const enum mode encrypting)
{
  ...
  if (data_processlen != 0)
  {
    ...
    memcpy(res,
           &data_process[(l - 1) * OMD_m],
           (data_process_modlen == 0 ? OMD_m : data_process_modlen));
    xor_block(res2, h, res);
    memcpy(&data[(l - 1) * OMD_m],
           res2,
           (data_process_modlen == 0 ? OMD_m : data_process_modlen));
    ...
    else if (encrypting == OMD_DECRYPT)
    {
      if (statedelta == 2)
      {
        ...
        memset(&res2[data_process_modlen + 1],
               0x00,
